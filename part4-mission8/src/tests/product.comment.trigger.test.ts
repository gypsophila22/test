...
import type { PrismaPatchProduct } from './types/prisma-partial.js';

jest.setTimeout(15000);

test('상품: 남의 글에 댓글 → 글쓴이에게 알림', async () => {
  await jest.isolateModulesAsync(async () => {
    // 1) prisma 싱글톤 로드 (모듈 타입은 .d.ts에서 제공)
    const { prisma } = await import('../lib/prismaClient.js');

    // 2) 우리가 덮어쓸 부분만 타입 안전하게 패치
    const patch: PrismaPatchProduct = {
      product: {
      ...
      notification: {
        create: jest.fn(async (args: Prisma.NotificationCreateArgs) => ({
          ...
          createdAt: new Date(),
          articleId: null,
          productId: (args.data.productId as number | null) ?? null,
    ...
    Object.assign(prisma as unknown as PrismaPatchProduct, patch);
    ...
    const { productCommentService } = await import(
      '../services/comments/product-cmt-service.js'
    );

    // 5) 실행 & 검증
    const result = await productCommentService.createProductComment(9, 'hi', 2);
